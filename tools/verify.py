...
checks = {
    ...
    "schema": {"meta", "entries", "language_ngram", "word_unigram", "word_bigram", "traditional_map"}.issubset({row[0] for row in database.execute("SELECT name FROM sqlite_master WHERE type='table'")}),
    "language_model": database.execute("SELECT count(*) FROM language_ngram").fetchone()[0] >= 200_000,
    "word_unigram": database.execute("SELECT count(*) FROM word_unigram").fetchone()[0] >= 50_000,
    "word_bigram": database.execute("SELECT count(*) FROM word_bigram").fetchone()[0] >= 200_000,
    ...
    "sources": {
        "rime_ice", "zhwiki", "jingxing", "renfei_dict", "project_seed", "cc_cedict", "jieba", "thuocl",
        "sc_dictionary", "chinese_xinhua", "chinese_semantic_kb",
    ...
    "sc_dictionary_mask": database.execute("SELECT 1 FROM entries WHERE source_mask & 2048 != 0 LIMIT 1").fetchone() is not None,
    "xinhua_mask": database.execute("SELECT 1 FROM entries WHERE source_mask & 4096 != 0 LIMIT 1").fetchone() is not None,
    "semantic_kb_mask": database.execute("SELECT 1 FROM entries WHERE source_mask & 8192 != 0 LIMIT 1").fetchone() is not None,
    "xinhua_idiom": database.execute("SELECT 1 FROM entries WHERE word=? AND source_mask & 4096 != 0", ("阿鼻地狱",)).fetchone() is not None,
}
