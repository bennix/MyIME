...
\[
C(P)=a(n-\ell_P)+bN_{\mathrm{partial}}+cN_{\mathrm{typo}}+dN_{\mathrm{fuzzy}},
\]
...
&-\lambda_iR_{\mathrm{inc}}-\lambda_cR_{\mathrm{comp}}.
\end{align*}
...
\(R_{\mathrm{inc}}\) & 未消费输入量 & 字符或音节数 & 完整度检查 \\
\(R_{\mathrm{comp}}\) & 候选超出当前输入的音节量 & 非负整数 & 前缀补全长度差 \\
\bottomrule
\end{tabular}
\end{table}
...
\[
S_{\mathrm{new}}=S_{\mathrm{old}}+F(v)-\lambda_{\mathrm{word}}+B(u,v)+T(u,v)+U(v),
\]
