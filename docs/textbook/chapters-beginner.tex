...
\[
S(w)=\alpha\cdot \frac{f(w)}{65535}-\lambda\cdot\Delta_{\mathrm{comp}},
\]

其中 \(f(w)\) 是词库权重，\(\Delta_{\mathrm{comp}}\) 是候选比用户输入多出的音节数。下标 \(\mathrm{comp}\) 是 completion 的缩写；本书对具有语义的多字母下标统一使用直立 Roman 字体，避免与多个变量的乘积混淆。
